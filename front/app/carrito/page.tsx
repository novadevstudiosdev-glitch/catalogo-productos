...
export default function CarritoPage() {
    ...
    return (
      <section className="bg-background py-16 sm:py-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-2 text-muted-foreground">Agregá camisetas para verlas acá</p>
          <Link href="/productos">
  ...
  const whatsappMessage = items
    .map(
      (item) =>
        `- ${item.name} (${item.customization.size}, ${item.customization.color})${
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 space-y-4">
          {items.map((item, index) => (
            <div
              key={`${item.id}-${item.customization.size}-${item.customization.color}-${index}`}
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {item.customization.notes && (
                      <p className="text-xs text-muted-foreground italic">Notas: {item.customization.notes}</p>
                    )}
                  </div>
                  <button
                ...
                <div className="mt-auto flex items-center justify-between pt-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-md border border-border transition-colors hover:bg-secondary disabled:opacity-50"
                    >
        ...
        <div className="mt-8 rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            * La personalización puede afectar el precio final. Confirmamos por WhatsApp.
          </p>

          <Link
            href={`https://wa.me/5491112345678?text=${encodeURIComponent(fullWhatsappMessage)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-md bg-green-500 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-green-600"
          >
